 ...
 *
 */
abstract class LongestCommonSubsequence<VALUE> {
	private int[][] c;
	public int [][] b;
	...
	protected abstract boolean isInX(VALUE x);

	protected abstract boolean isInY(VALUE y);
	...
	public void calculateLcs() {
		...
		b = new int[lengthOfX() + 1][];
		for (int i = 0; i < c.length; i++) {
			c[i] = new int[lengthOfY() + 1];
			b[i] = new int[lengthOfY() + 1];
		}
	...
	public void printTable(){
		...
		for (int i=0; i<this.lengthOfX()+1; i++){
			for (int j=0; j<this.lengthOfY()+1; j++){
				System.out.print("[");
				System.out.print(b[i][j] == 1 ? "X" : " ");
				System.out.print("]");
			}
			System.out.println();
		}
	...
	public void diff(int i, int j) {
		...
		if (i > 0 && j > 0 && isXYEqual(i, j)) {
			...
			if (j > 0 && (i == 0 || c[i][j - 1] >= c[i - 1][j])) {
				...
				/**
				 * Check if the node exist in previous version
				 * YES -> a MOVE
				 * NO  -> an ADD
				 */

				VALUE v = valueOfYInternal(j);

				if (isInX(v)) {
					this.diff.add(new DiffEntry<VALUE>(DiffType.MOVE, v));
				} else {
					this.diff.add(new DiffEntry<VALUE>(DiffType.ADD, v));
				}
			} else if (i > 0 && (j == 0 || c[i][j - 1] < c[i - 1][j])) {
				diff(i - 1, j);

				/**
				 * Check if node exists in current version
				 * YES -> a MOVE
				 * NO  -> a REMOVE
				 */

				VALUE v = valueOfXInternal(i);

				if (isInY(v)) {
					// do nothing..since it is a move and it was taken by
					// the move.
				} else {
